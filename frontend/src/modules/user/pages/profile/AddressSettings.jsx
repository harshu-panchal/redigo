...
const AddressCard = ({
  ...
  <motion.div
    ...
    <div
      className={`w-12 h-12 rounded-2xl border border-white/80 bg-white/70 shadow-sm flex items-center justify-center shrink-0 ${accentClass}`}>
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <div
            className={`mt-1 text-[12px] font-bold ${isEmpty ? "text-slate-400 italic" : "text-slate-500"} truncate`}>
            {subtitle}
          </div>
        </div>
...
const AddressSettings = () => {
  ...
  const draftDefaults = useMemo(() => {
    ...
    if (mode === "work")
      return (
        data.work || { label: "Work", address: "", landmark: "", notes: "" }
      );
    if (mode === "landmark") {
  ...
  const saveDraft = () => {
    ...
    if (modal.mode === "home") {
      setData((prev) => ({
        ...prev,
        home: { ...prev.home, ...draft, label: "Home" },
      }));
      closeModal();
      return;
    }
  ...
  const doDelete = () => {
    ...
    if (confirmDelete.mode === "home") {
      setData((prev) => ({
        ...prev,
        home: { ...prev.home, address: "", landmark: "", notes: "" },
      }));
      ...
      setData((prev) => ({
        ...prev,
        landmarks: prev.landmarks.filter((l) => l.id !== confirmDelete.id),
      }));
    }
  ...
  const homeSubtitle = data.home?.address?.trim()
    ? data.home.address
    : "Add your home address";
  const workSubtitle = data.work?.address?.trim()
    ? data.work.address
      ...
      <header className="sticky top-0 z-30">
        <div className="bg-white/70 backdrop-blur-md border-b border-white/70 shadow-[0_10px_20px_rgba(15,23,42,0.05)]">
          <div className="px-5 py-4 flex items-center gap-3">
            <button
              onClick={() => navigate(-1)}
              className="p-2 -ml-2 active:scale-95 transition-all rounded-full">
      ...
      <div className="relative z-10 px-5 pt-5 space-y-5 flex-1">
        <div className="space-y-3">
          <AddressCard
            ...
            onDelete={() =>
              setConfirmDelete({ mode: "home", title: "Home address" })
            }
          />
          <AddressCard
            icon={Briefcase}
            ...
            onDelete={() =>
              setConfirmDelete({ mode: "work", title: "Work address" })
            }
          />
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-end justify-between px-1">
            <div>
              ...
              <div className="mt-1 text-[15px] font-black text-slate-900 tracking-tight">
                Saved places
              </div>
            </div>
            <button
          ...
          {hasLandmarks ? (
            <div className="bg-white/75 backdrop-blur-md rounded-[22px] border border-white/80 shadow-[0_14px_34px_rgba(15,23,42,0.06)] overflow-hidden">
              {data.landmarks.map((lm) => (
                <div
                  key={lm.id}
                  className="px-4 py-3 border-b border-white/70 last:border-none flex items-start gap-3">
