...
const SenderReceiverDetails = () => {
  ...
  const validate = () => {
    ...
    if (!PHONE_REGEX.test(senderMobile)) {
    ...
    if (!PHONE_REGEX.test(receiverMobile)) {
      newErrors.receiverMobile = 'Enter a valid 10-digit mobile number';
    }
    setErrors(newErrors);
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] max-w-lg mx-auto flex flex-col font-sans relative">
      <header className="bg-white px-5 py-6 flex items-center gap-6 border-b border-gray-50 shadow-sm sticky top-0 z-20">
        ...
        <div className="ml-auto rounded-full border border-gray-100 bg-gray-50 px-3 py-1.5 text-[10px] font-black text-gray-500 shadow-sm shrink-0">
          Step 4 of 4
        </div>
      </header>
      ...
      <div className="flex-1 p-5 space-y-8 overflow-y-auto no-scrollbar pb-4">
        <div className="space-y-4">
          <div className="flex items-center gap-3 ml-1">
            ...
            <h3 className="text-[16px] font-black text-gray-900 tracking-tight leading-none">
              Sender Details
            </h3>
          </div>
        ...
        <div className="space-y-4 pb-24">
          ...
          {parcelState.estimatedFare && (
            <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100 flex justify-between items-center gap-4">
              <div>
                <span className="text-[12px] font-black text-gray-500 uppercase tracking-widest">
                  Est. Delivery Fare
                </span>
                {parcelState.vehicle?.name ? (
                  <p className="mt-1 text-[11px] font-bold text-gray-400">
                    {parcelState.vehicle.name}
                  </p>
                ) : null}
              </div>
              <span className="text-[16px] font-black text-gray-900 text-right">
                Rs {parcelState.estimatedFare.min}-Rs {parcelState.estimatedFare.max}
              </span>
            </div>
          )}
        </div>
