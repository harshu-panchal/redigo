    ...
    timerRef.current = setTimeout(() => {
      ...
      timerRef.current = setTimeout(() => {
        ...
        timerRef.current = setTimeout(() => {
          ...
          setTimeout(() => navigate('/parcel/tracking', { 
            state: { 
              ...routeState, 
              otp, 
              driver, 
              fare: routeState.fare || routeState.estimatedFare?.min || routeState.vehicle?.price || 45, 
          ...
          {isSearching && (
            <motion.div key="searching" initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }}
              ...
              <div className="text-center space-y-0.5">
                ...
                <p className="text-[11px] font-bold text-slate-400">Connecting with agents nearby</p>
              </div>
              ...
              <div className="flex items-center justify-center gap-4 py-2 border-y border-slate-50">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-wider">Fast Delivery</span>
                </div>
                <div className="w-px h-3 bg-slate-100" />
                <div className="flex items-center gap-1.5">
                  <ShieldCheck size={11} className="text-blue-500" strokeWidth={2.5} />
                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-wider">Secure Parcel</span>
                </div>
              </div>
          ...
          {isAssigned && (
            <motion.div key="assigned" initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }}>
              <DriverCard driver={driver}
                bannerGradient="bg-gradient-to-r from-blue-500 to-blue-600"
                banner={<>
                  <CheckCircle2 size={16} className="text-white shrink-0" strokeWidth={2.5} />
                  <div className="flex-1">
                    <p className="text-white font-black text-[13px] leading-tight">Delivery Agent Found!</p>
                    <p className="text-blue-100 text-[10px] font-bold">Waiting for agent to accept request...</p>
                  </div>
                ...
                <div className="flex items-center gap-3 rounded-[12px] border border-dashed border-slate-200 bg-slate-50/60 px-3 py-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] font-bold text-slate-400">Generates securely upon acceptance.</p>
                  </div>
                </div>
            ...
            <motion.div key="accepted" initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }}>
              <DriverCard driver={driver}
                ...
                banner={<>
                  ...
                  <div>
                    <p className="text-white font-black text-[13px] leading-tight">Delivery Confirmed!</p>
                    <p className="text-orange-100 text-[10px] font-bold">Your agent will arrive shortly.</p>
                  </div>
                </>}>
                <div className="flex gap-2">
                  <ActionBtn icon={Phone} label="Call" onClick={() => window.open(`tel:${driver.phone}`)} />
                ...
                <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-slate-400 mt-0.5">Used to verify handover.</p>
                  </div>
                  <div className="flex gap-1">
                    {otp.split('').map((d, i) => (
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Preparing Navigation...</span>
                </div>
              </DriverCard>
            </motion.div>
          )}
          ...
          <>
            ...
            <motion.div initial={{ scale: 0.92, opacity: 0, y: 40 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.92, opacity: 0, y: 40 }}
              ...
              <p className="text-[13px] font-bold text-slate-400 mb-6 leading-relaxed">
                {isAssigned ? 'A delivery agent has been assigned. Are you sure you want to cancel?' : "We're currently searching for an agent. Stop search?"}
              </p>
